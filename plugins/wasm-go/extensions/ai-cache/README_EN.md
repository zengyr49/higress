...
| responsesResponseTemplate | string | optional | `{"id":"from-cache","object":"response","status":"completed","model":"from-cache","output":[{"type":"message","role":"assistant","content":[{"type":"output_text","text":"%s"}]}],"usage":{"input_tokens":0,"output_tokens":0,"total_tokens":0}}` | Template for returning OpenAI Responses API HTTP response, with %s marking the part to be replaced by cache value |
| responsesStreamResponseTemplate | string | optional | `event: response.output_text.delta\ndata: {"type":"response.output_text.delta","delta":"%s"}\n\nevent: response.completed\ndata: {"type":"response.completed","response":{"id":"from-cache","object":"response","status":"completed","model":"from-cache","output":[{"type":"message","role":"assistant","content":[{"type":"output_text","text":"%s"}]}],"usage":{"input_tokens":0,"output_tokens":0,"total_tokens":0}}}\n\n` | Template for returning OpenAI Responses API streaming HTTP response, with %s marking the part to be replaced by cache value. The default template uses the cache value twice |

When the request path ends with `/responses`, the plugin handles OpenAI Responses API caching automatically: the default cache key is extracted from `input`, non-streaming responses aggregate `text` fields from `output[].content[]` entries whose `type` is `output_text`, and streaming responses aggregate `delta` from `response.output_text.delta` events. If a streaming response contains function/tool call events, the plugin skips writing that response to cache.
