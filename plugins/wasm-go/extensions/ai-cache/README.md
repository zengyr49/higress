...
| responsesResponseTemplate | string | optional | `{"id":"from-cache","object":"response","status":"completed","model":"from-cache","output":[{"type":"message","role":"assistant","content":[{"type":"output_text","text":"%s"}]}],"usage":{"input_tokens":0,"output_tokens":0,"total_tokens":0}}` | 返回 OpenAI Responses API HTTP 响应的模版，用 %s 标记需要被 cache value 替换的部分 |
| responsesStreamResponseTemplate | string | optional | `event: response.output_text.delta\ndata: {"type":"response.output_text.delta","delta":"%s"}\n\nevent: response.completed\ndata: {"type":"response.completed","response":{"id":"from-cache","object":"response","status":"completed","model":"from-cache","output":[{"type":"message","role":"assistant","content":[{"type":"output_text","text":"%s"}]}],"usage":{"input_tokens":0,"output_tokens":0,"total_tokens":0}}}\n\n` | 返回 OpenAI Responses API 流式 HTTP 响应的模版，用 %s 标记需要被 cache value 替换的部分。默认模板会使用两次 cache value |

当请求路径以 `/responses` 结尾时，插件会按 OpenAI Responses API 自动处理缓存：请求缓存键默认从 `input` 提取，非流式响应默认从 `output[].content[]` 中 `type=output_text` 的 `text` 字段聚合，流式响应默认聚合 `response.output_text.delta` 事件中的 `delta` 字段。若流式响应中出现 function/tool call 事件，插件会跳过该响应的缓存写入。
